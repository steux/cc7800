...
const char custom_dl[12] = {display_score_str, 0x60, display_score_str >> 8, _ms_width_palette(5, 3), 0, lives, 0xc0, lives >> 8, _ms_width_palette(12, 0), 160 - 8 * 3, 0, 0};
ramchip char dl_in_ram[12];

inline void update_lives_display()
{
    dl_in_ram[8] = (-(nb_lives << 2)) & 0x1f;
}
...
INIT_BANK void init()
{
    ...
    // Copy custom_dl to RAM
    for (X = 11; X >= 0; X--) {
        dl_in_ram[X] = custom_dl[X];
    }

    // Score display on the top zone
    multisprite_set_top_zone_dl(0, dl_in_ram);
...
{
    my_display_big_sprite(x, y, boss, 12, palette, 3, 1);
}
...
void draw_gameover()
{
    ...
    my_display_big_sprite((80 - 16), ((224 - 16 - 48 - 48) / 2), skull, 8, 0, 3, 0);
    x = xx - 36;
    ptr = game;
    for (i = 0; i != 2; i++) {
...
void step()
{
    ...
    if (draw_spaceship) {
        my_display_big_sprite(spaceship_xpos, spaceship_ypos, spaceship, 6, 0, 2, 1);
        // Draw exhaust
    ...
    for (i = big_explosion_first; i != big_explosion_last; i++) {
        ...
        if (big_explosion_counter[X]) {
            ...
            my_display_big_sprite(xbig_explosion, ybig_explosion, gfx, 6, 3, 3, 0);
        } else {
